...
/**
 * @fileoverview Templates in namespace Confluence.SourceEditor.Templates.
 */
...
define('source-editor/source-editor-cm-search-adapter', [
], function (
) {
    ...
    return function (editor) {
        ...
        function clearState() {
            state.from = state.to = e.getCursor();
            state.query = '';
        }
        ...
        function find(options) {
            ...
            if (cursor) {
                ...
                if (!match && options.wrap) {
                    // try from start / end
                    cursor = e.getSearchCursor(state.query, options.backward ? { line: e.lineCount() - 1 } : { line: 0, ch: 0 }, !state.matchCase);
                    match = cursor.find(options.backward);
                }
                if (match) {
                    saveState();
        ...
        function replace(text, wrap) {
            if (cursor) {
                ...
                cursor.replace(text);
                return find({ backward: false, wrap: wrap });
            }
        }
...
;
try {
/* module-key = 'com.atlassian.confluence.plugins.editor.confluence-source-editor:atlassian-source-editor-javascript', location = 'jscripts/source-editor/editor-integration.js' */
/*
 * Copyright © 2012 - 2013 Atlassian Corporation Pty Ltd.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
...
require([
    ...
    Event.bind('init.rte', SourceEditor.init);
});
